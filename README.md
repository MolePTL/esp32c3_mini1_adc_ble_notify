...
- PC 上位机：使用 `PySide6 + bleak + pyqtgraph + matplotlib` 实现 BLE 连接、协议解析、实时示波、滤波换算、CSV 保存、历史波形回放和论文图片导出。
...
- 支持记录过程中打开独立“记录回看”窗口，查看已经写入实时 CSV 的 `10 Hz` 聚合数据
- 支持导入单个 CSV 或整个 CSV 文件夹，绘制长时间总波形
- 历史导入默认优先使用滤波后的 `*_voltage_filtered_v` 列，缺失通道会自动隐藏
- 历史 CSV 导入后可导出适合插入 Word 的高 DPI PNG 图片，支持选择纵轴数据列、横轴单位和图片尺寸
...
│  ├─ image_export_dialog.py # 图片导出参数对话框
│  ├─ plot_image_exporter.py # 论文风格 PNG 导出
│  ├─ record_review_window.py # 实时记录 CSV 回看窗口
...
7. 实时保存开启后，可点击“查看记录”打开独立窗口回看已经写入 CSV 的聚合数据
...
6. 点击“导出图片”可生成论文/报告风格 PNG；导出前可选择纵轴数据、横轴单位（秒/分钟/小时）、Word 尺寸预设或自定义厘米尺寸与 DPI

“导出图片”使用当前示波器的可见时间范围。需要导出完整曲线时，先点击“显示全部”；需要局部图片时，先拖动或缩放到目标时间段。
