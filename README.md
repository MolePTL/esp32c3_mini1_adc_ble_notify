...
这是一个基于 `ESP32-C3` 的四路 ADC 采样与 BLE Notify 数据采集项目，配套提供 Windows 上位机，用于扫描设备、连接订阅、实时显示波形、处理传感器数据、保存 CSV 数据，并把历史 CSV 导回上位机绘制总波形。
...
| `VA201` | `GPIO3` | `ADC_CHANNEL_3` | FlexiForce A201 相关电压采样 |
| `VBAT` | `GPIO4` | `ADC_CHANNEL_4` | 电池分压采样 |
| `WS2812` | `GPIO8` | - | 单颗状态灯 |
...
上位机中的传感器换算常量集中在 [`pc_app/protocol.py`](pc_app/protocol.py)。当前 `VA201` 按 A201 运放输出电压反算等效阻值，`VBAT` 按 `39kΩ / 100kΩ` 分压还原电池端电压。
