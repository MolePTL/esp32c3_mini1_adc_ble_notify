...
这是一个基于 `ESP32-C3` 的四路 ADC 采样与 BLE Notify 演示项目，配套提供 Windows 上位机，用于扫描设备、连接订阅、实时显示波形、保存 CSV 数据，并把历史 CSV 导回上位机绘制总波形。

项目当前由两部分组成：

- 固件端：周期采样 4 路 ADC，并通过自定义 GATT Service/Characteristic 按固定 20 字节协议持续发送。
- PC 上位机：使用 `PySide6 + bleak + pyqtgraph` 实现 BLE 连接、协议解析、实时示波、CSV 保存和历史波形回放。
